...
class Stream():
    ...
    def check_bandwidth(self, target_bandwidth, num_readings=3, tolerance=0.05, num_ignored_readings=0):
        ...
        if (current_bandwidth >= (1.0 - tolerance) * target_bandwidth
            ...
            logger.error("%s Expected %.1f Kbps but measured %.1f Kbps.",
                         self.name, float(target_bandwidth), current_bandwidth)
            raise AssertionError("{0} Expected {1:.1f} Kbps but measured {2:.1f} Kbps.".format(
                self.name, float(target_bandwidth), current_bandwidth))
...
class Sender(l2tester.Sender):
    ...
    def start(self):
        ...
        """
        logger.info("Sending %d Kbps from %s interface.",
                    self.get_bandwidth()/1000, self.iface_name)
