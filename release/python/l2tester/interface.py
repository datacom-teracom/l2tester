...
import socket, struct, fcntl, ctypes, os, re
import logging
...
class Interface():
	...
	def __init__(self, name, eth_type=ETH_P_ALL):
		...
		if not self.netlink.link_lookup(ifname=self.name):
			vlan_match = re.match ("^(?P<base_interface>eth\d+)\.(?P<vlan_id>[1-9]\d{1,3})$", self.name)
	...
	def force_speed_duplex(self, speed, duplex):
		...
		self.logger.info("[%s] Configured forced speed: %d Mbps / %s duplex", self.name, speed, "full" if duplex else "half")
	...
	def __check_ip_address(self, ip_address, ip_family):
		...
		"""
		address_types = ['IFA_ADDRESS', 'IFA_LOCAL', 'IFA_BROADCAST', 'IFA_ANYCAST', 'IFA_MULTICAST']
	...
	def __set_ip_address(self, ip_address, ip_family, action):
		...
		self.logger.info ("%s IPv%d address %s/%d in %s", action, ip_version, ip_and_mask[0], network_mask, self.name)
		self.netlink.addr(action, self.if_index, address = ip_and_mask[0], mask = network_mask, family = ip_family)
