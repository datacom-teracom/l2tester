...
def frame_mapping_to_str(mapping):
	...
	return '\n'.join([
	" {0:<9} : {1}".format(iface, '\n           : '.join([
		"{0} ({1} bytes) [x{2}]".format(Ether(frame).summary(), len(frame)+4, count) # Add 4 bytes of FCS
...
def send_frame(frame_mapping):
	...
	if type(frame_mapping) != dict:
		raise TypeError("Expected dictionary for frame_mapping, got " + str(type(frame_mapping)) + ".")
...
def send_and_check(send_frames, expected_frames, ignore_unexpected_frames = False, timeout = 1.0, do_not_log_error_on_error = False):
	...
	if not missed_frames:
		...
		if received_frames:
			logger.info("Received %d of %d expected frames:\n%s", num_received, num_expected, frame_mapping_to_str(received_frames))
		err_msg_missed = "Missed {0} expected frames:\n{1}".format(num_missed, frame_mapping_to_str(missed_frames))
	...
	if (unexpected_frames and not ignore_unexpected_frames):
		...
		err_msg_unexpected = "Received {0} unexpected frames:\n{1}".format(num_unexpected, frame_mapping_to_str(unexpected_frames))
		if do_not_log_error_on_error:
